...
def get_submeshes(file, index, pos_verts, uv_verts, weights, jud_vcs, vert_colours, face_hex, jud_shader, obfuscate, cloth_override):
    ...
    for submesh in list(submeshes):
        ...
        submesh.pos_verts = trim_verts_data(pos_verts, dsort)
        if uv_verts:
            submesh.uv_verts = trim_verts_data(uv_verts, dsort)
        else:
            submesh.uv_verts = uv_verts

        if weights:
            submesh.weights = trim_verts_data(weights, dsort)
        else:
            submesh.weights = []
    ...
    for i, w in enumerate(weights_arrs):
        ...
        for j in range(len(indices)):
            if len(bone_cluster) < indices[j]:
                print(f'Bone index longer than bone clusters, could not add weights ({indices[j]} > {len(bone_cluster)})')
                return
            try:
...
def get_model(parent_file, all_file_info, hash64_table, temp_direc='', lod=True, b_textures=False, b_temp_direc_full=False, obfuscate=False, b_apply_textures=False, passing_dyn3=False, b_skeleton=False, from_api=False, b_shaders=False, jud_shader=False, custom_export=False, b_collect_extra_textures=False):
    b_verbose = True
    if custom_export:
        model = custom_export
    else:
    ...
    if passing_dyn3:
        ...
        if b_verbose:
            print(f'Dyn1 {parent_file}')
        fbdyn1 = open(f'I:/d2_output_3_1_0_0/{gf.get_pkg_name(parent_file)}/{parent_file}.bin', 'rb').read()
        ...
        for d, dyn2 in enumerate([dyn2_primary, dyn2_secondary, dyn2_ternary]):
            if not dyn2 or dyn2 not in all_file_info.keys() or dyn2 == '0400-0000':
                ...
                continue
            fbdyn2 = open(f'I:/d2_output_3_1_0_0/{gf.get_pkg_name(dyn2)}/{dyn2}.bin', 'rb').read()
            ...
            for i, pos_vert_file in enumerate(pos_verts_files):
                ...
                if not custom_export:
                    if lod:
                        ...
                        add_to_fbx(model, bones, submeshes, parent_file, pos_vert_file.uid.upper(), temp_direc, b_temp_direc_full, b_apply_textures, b_textures, skel_file, obfuscate, existing_mats, b_shaders, all_file_info, b_verbose, hash64_table)
                all_submeshes.append(submeshes_to_write)
            model_files.append(model_file)
    if model_files:
...
def collect_extra_textures(fb, temp_direc, b_verbose, hash64_table, all_file_info):
    offset = fb.find(b'\x14\x00\x80\x80')
    if offset == -1:
        if b_verbose:
            print('No extra textures')
        return
    count = gf.get_uint32(fb, offset-8)
    offset += 8
    gf.mkdir(f'I:/dynamic_models/{temp_direc}/unk_textures/')
    mats = list(set([fb[i:i+4] for i in range(offset, offset+count*4, 4)]))
    for m in mats:
        mat = gf.get_file_from_hash(m.hex())
        if mat == 'FBFF-1FFF':
            continue
        mfb = open(f'I:/d2_output_3_1_0_0/{gf.get_pkg_name(mat)}/{mat}.bin', 'rb').read()
        off = mfb.find(b'\xCF\x6D\x80\x80')
        if off == -1:
            continue
        c = gf.get_uint32(mfb, off-8)
        # Arbritrary
        if c <= 0 or c > 100:
            continue
        if b'\xFF\xFF\xFF\xFF' in mfb[off + 0x10:off + 0x10 + 24 * count]:  # Uses new texture system
            textures = [x for x in [gf.get_file_from_hash(hash64_table[mfb[off + 8 + 0x10 + 24 * i:off + 8 + 0x10 + 24 * i + 8].hex().upper()]) for i in range(c)] if x != 'FBFF-1FFF']
        else:
            textures = [x for x in [gf.get_file_from_hash(mfb[off + 0x10 + 24 * i:off + 0x10 + 24 * i + 4].hex().upper()) for i in range(c)] if x != 'FBFF-1FFF']
        for img in textures:
            if not os.path.exists(f'I:/dynamic_models/{temp_direc}/unk_textures/{img}.tga'):
                imager.get_image_from_file(f'I:/d2_output_3_1_0_0/{gf.get_pkg_name(img)}/{img}.bin', all_file_info, f'I:/dynamic_models/{temp_direc}/unk_textures/')
...
def add_to_fbx(model, bones, submeshes, parent_file, name, temp_direc, b_temp_direc_full, b_apply_textures, b_textures, skel_file, obfuscate, existing_mats, b_shaders, all_file_info, b_verbose, hash64_table):
    ...
    for submesh in submeshes:
        mesh = create_mesh(model, submesh, name, skel_file and submesh.weights and bones)
        ...
        if submesh.material.uid != 'FFFFFFFF':
            ...
            if b_shaders:
                ...
                get_shader_info(submesh, temp_direc, all_file_info)
...
def get_shader_info(submesh, temp_direc, all_file_info):
...
def try_get_material_textures(submesh, temp_direc, b_apply_textures, hash64_table, all_file_info):
    ...
    for img in submesh.textures:
        ...
        # imager.get_image_from_file(f'I:/d2_output_3_1_0_0//{gf.get_pkg_name(img)}/{img}.bin', all_file_info, f'I:/dynamic_models/{temp_direc}/textures/{img}.dds')
            imager.get_image_from_file(f'I:/d2_output_3_1_0_0//{gf.get_pkg_name(img)}/{img}.bin', all_file_info, f'I:/dynamic_models/{temp_direc}/textures/')
...
def parse_skin_buffer(verts_file, all_file_info, skin_file):
    ...
    out_weights = []
    ref_file = gf.get_file_from_hash(all_file_info[verts_file.name]['Reference'])
    verts_fb = open(f'I:/d2_output_3_1_0_0/{gf.get_pkg_name(ref_file)}/{ref_file}.bin', 'rb').read()
    ...
    skin_fb = open(f'I:/d2_output_3_1_0_0/{gf.get_pkg_name(ref_file)}/{ref_file}.bin', 'rb').read()
    ...
    for w in verts_w:
        ...
        if blend_flags & 0x8000:
            ...
            print('Unk flag used in skin buffer')
            return
...
def create_mesh(model, submesh: Submesh, name, skel):
...
def export_all_models(pkg_name, all_file_info, select, lod_filter, b_textures, b_skeleton, obfuscate=False):
    ...
    for file in list(entries_type.keys()):
        ...
        get_model(file, all_file_info, hash64_table, lod=lod_filter, temp_direc=pkg_name, passing_dyn3=False, obfuscate=obfuscate,
...
if __name__ == '__main__':
    ...
    select = '02f7'
    folder = select
    for pkg in pkg_db.get_all_tables()[::-1]:
        if select in pkg:
            # if 'weapons' in pkg:
            #     continue
            gf.mkdir(f'I:/dynamic_models/{pkg}')
            # if pkg not in os.listdir('C:/d2_model_temp/texture_models/tower'):
            export_all_models(pkg, all_file_info, folder, lod_filter=True, b_textures=True, b_skeleton=False, obfuscate=False)
