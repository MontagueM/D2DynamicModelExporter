...
class TexturePlateSet:
    ...
    def get_plates(self, direct_from_tex):
        if not direct_from_tex:
            fb = open(f'I:/d2_output_3_0_1_3/{gf.get_pkg_name(self.topfile)}/{self.topfile}.bin', 'rb').read()
            # This offset is a guess for now
            offset = fb.find(b'\xCD\x9A\x80\x80')+8
            dyn2 = gf.get_file_from_hash(bytes.hex(fb[offset:offset+4]))
            fb = open(f'I:/d2_output_3_0_1_3/{gf.get_pkg_name(dyn2)}/{dyn2}.bin', 'rb').read()
...
class TexturePlate:
    ...
    def get_plate_data(self):
        if self.file == 'FBFF-1FFF':
            return False
        fb = open(f'I:/d2_output_3_0_1_3/{gf.get_pkg_name(self.file)}/{self.file}.bin', 'rb').read()
